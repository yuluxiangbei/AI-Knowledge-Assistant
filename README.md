...
前后端分离的全栈项目,从零实现完整的 **登录认证 → 文档管理 → RAG 问答 → 引用溯源** 链路。前端为 Vue 3 + Vite 的 ChatGPT 风格界面,后端为 FastAPI 异步架构。每一课的知识点与踩坑记录在 [项目历程.md](项目历程.md)。

## ✨ 功能特性

**前端(Vue 3,`frontend-vue/`)**
- ChatGPT 风格界面:左侧会话栏 + 右侧聊天区
- 登录 / 注册(token 存 localStorage,刷新不掉线)
- 文档上传 / 列表 / 删除(后端联动清理向量)
- 多会话管理:新建 / 切换 / 删除
- 聊天问答 + **引用来源标签展示**(assistant 回答附带)
- 交互细节:Enter 发送、Shift+Enter 换行、发送中防重复

**后端(FastAPI)**
- JWT 无状态认证 + bcrypt 密码哈希
- 文档上传三道校验(类型 / 大小 / 数量)→ 解析 → 滑动窗口切片 → BGE 向量化入库
- RAG 链路:检索 top-k → 拼 prompt → DeepSeek 生成,带对话历史上下文
...
┌───────────────────┐   HTTP + Bearer Token   ┌──────────────────────────────────────────┐
│   Vue 3 前端      │ ───────────────────────► │                 FastAPI                  │
│  (Vite, :5173)    │ ◄─────────────────────── │  JWT 认证 → 用户/会话/消息/文档(MySQL)   │
│  登录/上传/聊天   │      JSON + CORS         │                                        │
│  引用来源展示     │                          │  上传: 解析 → 切片 → BGE编码 → Qdrant    │
...
# 首次运行 BGE 模型需联网下载;国内可设 HF_ENDPOINT=https://hf-mirror.com
```
...
打开 <http://localhost:5173> 注册账号 → 上传文档 → 新建会话提问,即可看到带引用来源的回答。

> 💡 国内网络提示:模型下载走 HuggingFace 镜像 `HF_ENDPOINT=https://hf-mirror.com`;若 qdrant-client 请求 localhost 报 502(代理劫持),设置 `no_proxy=localhost,127.0.0.1`。后端 API 文档:<http://127.0.0.1:8000/docs>。
...
项目从零开始开发,共十课(最小应用 → 分层架构 → 数据库 → 认证 → 会话 → 文档解析 → 向量检索 → RAG 链路 → 接口接入 → Vue 3 前端),每一课的知识点、踩坑与验证方式记录在 [项目历程.md](项目历程.md)。

## 📝 TODO

- [ ] SSE 流式输出(打字机效果)
- [ ] 自动化测试补全
- [ ] Docker 化部署(前后端 + MySQL + Qdrant 一键起)
